...
class Environment(object):
    ...
    def record_timestep_stats(self, result):
        ...
        if self.verbose >= 1 and self.stats_file is None:
            self.stats_file = open(('snake-env-{timestamp}.csv').format(timestamp=timestamp), 'w')
        ...
        if self.verbose >= 2 and self.debug_file is None:
            self.debug_file = open(('snake-env-{timestamp}.log').format(timestamp=timestamp), 'w')
...
class TimestepResult(object):
    ...
    def __str__(self):
        ...
        return ('{field_map}\nR = {reward}   end={is_episode_end}\n').format(field_map=field_map, reward=self.reward, is_episode_end=self.is_episode_end)
...
class EpisodeStatistics(object):
    ...
    def flatten(self):
        ...
        flat_stats.update({
            ('action_counter_{action}').format(action=self.action_counter.get(action, 0))
            for action in ALL_SNAKE_ACTIONS
        })
        return flat_stats
